...
export const createLeftSlope = (layer, indices, x, y, scene, height = 1, soilIndex = 20, grassIndex = 2) => {
    let diff;
    const xLeft = x;
    const yLeft = y;
    ...
    var graphics = scene.add.graphics({ lineStyle: { width: 2, color: 'black' } });
...
export const createRightSlope = (layer, indices, x, y, scene, height = 1, soilIndex = 20, grassIndex = 2) => {
    let diff;
    const xLeft = x;
    const yLeft = y;
    for (let i = height; i > 0; i--) {
